...
%   unique: bool flag to use unique time points only (1 time per stationary
%           configuration regardless of stationary duration)
    ...
    if sum(data.bflying(t,:)) == 0 % No bats are flying
        ...
        if(is_dupe)
            ...
            if(t1 - t0 > 100)
                ...
                stat_period_lengths(end + 1) = t1 - t0;
                is_dupe = false;
            end
        end
    end
...
if unique
    ...
    time_points = stat_times;
end
disp(size(time_points));
disp(size(stat_period_lengths));
...
if(run_tests)
    ...
    bat_areas = zeros(length(time_points),5);
...
if(show_figures)
    figure(5);
    F = getframe;
    for i=1:length(time_points(1:60))
        t = time_points(i);
        P = cat(2, x_pos(:,t),y_pos(:,t));
        [A, b, V] = voronoiPolyhedronsCCW(P.', [-2.8 -2.8], [2.8 2.8]);
        for j=1:length(V)
...
voronoi_areas = area;
stationary_durations = stat_period_lengths;
end
